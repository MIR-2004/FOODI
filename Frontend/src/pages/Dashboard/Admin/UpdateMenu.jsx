...
import { Save, UtensilsCrossed, DollarSign, FileText, Image, Tag, Sparkles, Leaf } from "lucide-react";
...
const UpdateMenu = () => {
  ...
  const { register, handleSubmit, reset, watch } = useForm({
    defaultValues: { isVeg: item.isVeg === true ? "true" : item.isVeg === false ? "false" : undefined }
  });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        <div className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-brand-gold/10 border border-brand-gold/20 rounded-full text-[10px] font-bold tracking-wider text-brand-gold uppercase mb-2">
          <Sparkles className="h-2.5 w-2.5" /> Edit Item
        </div>
        <h1 className="text-2xl font-extrabold text-white tracking-tight">
          Update <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-gold to-amber-400">Menu Item</span>
        </h1>
        <p className="text-slate-400 text-sm mt-1">
          Editing: <span className="text-slate-300 font-semibold">{item.name}</span>
        </p>
      </div>
      ...
      <div className="bg-[#0d1221]/80 border border-slate-800/60 rounded-2xl p-6 md:p-8">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              <Image className="h-3.5 w-3.5 text-purple-400" /> Update Photo
            </label>
            {item.image && (
              <div className="w-20 h-20 rounded-xl overflow-hidden border border-slate-800/60 mb-2">
